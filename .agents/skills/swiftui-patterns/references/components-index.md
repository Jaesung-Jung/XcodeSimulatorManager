...
- Observation models: `references/observation.md` — Use for `@Observable` feature models, reference state, and child bindings.
- State ownership: `references/state-ownership.md` — Use to choose between `@State`, `@Binding`, `@Bindable`, and environment injection.
- Environment with Observation: `references/environment-observation.md` — Use for `environment(_:)` and `@Environment(Type.self)` on iOS 17+.
- Observation migration: `references/observation-migration.md` — Use when converting `ObservableObject` and legacy wrappers to Observation.
- Latest APIs: `references/latest-apis.md` — Use when modernizing legacy SwiftUI code or replacing deprecated APIs.
...
- Modern view modifiers: `references/view-modifiers.md` — Use for `onChange(of:initial:_:)`, `.task`, and `.task(id:)`.
- View composition: `references/composition.md` — Use when splitting large views into focused subviews or reusable modifiers.
- Accessibility: `references/accessibility.md` — Use for VoiceOver, Dynamic Type, and accessible interactive surfaces.
- UIKit interoperability: `references/uikit-interop.md` — Use when wrapping UIKit views or controllers in SwiftUI.
