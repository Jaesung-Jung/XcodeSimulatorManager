...
description: Refactor an existing SwiftUI view file while intentionally preserving behavior. Use only for structure-only cleanup such as splitting long bodies, extracting dedicated subviews, removing inline actions or side effects, stabilizing the view tree, or standardizing existing Observation and view-model initialization patterns. Do not use this for greenfield SwiftUI feature work or intentional behavior changes.
---

# SwiftUI View Refactor

## Overview
Refactor SwiftUI views toward small, explicit, stable view types without broad behavior changes. This skill is for structure cleanup of an existing file, not for general feature design, Liquid Glass adoption, runtime performance investigation, or intentional UX/behavior changes.

Use another SwiftUI skill when the primary goal is:
- New feature design or general implementation -> `swiftui-patterns`
- Runtime performance diagnosis -> `swiftui-performance-audit`
