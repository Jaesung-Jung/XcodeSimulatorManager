...
assert_max_lines \
  "CoreSimulatorClient.swift는 public endpoint contract와 initializer만 남겨 180줄 이하로 유지합니다." \
  180 \
  "$PACKAGE_SOURCES/SimControlClients/CoreSimulatorClient/CoreSimulatorClient.swift"
...
assert_max_lines \
  "DeviceLifecycleWorkflowClient.swift는 public model/endpoint contract와 live assembly만 남겨 180줄 이하로 유지합니다." \
  180 \
  "$PACKAGE_SOURCES/MainWindowWorkflows/DeviceLifecycleWorkflowClient.swift"
...
assert_max_lines \
  "CreateDeviceView.swift는 create-device sheet root 조합만 남겨 180줄 이하로 유지합니다." \
  180 \
  "$PACKAGE_SOURCES/MainWindowSheetsFeature/CreateDevice/CreateDeviceView.swift"
...
assert_max_lines \
  "MainWindowFeature+StateDeviceLifecycle.swift는 device lifecycle 상태 projection만 남겨 90줄 이하로 유지합니다." \
  90 \
  "$PACKAGE_SOURCES/MainWindowFeature/MainWindow/Features/MainWindowFeature+StateDeviceLifecycle.swift"

assert_max_lines \
  "MainWindowFeature+DeveloperTools.swift는 developer tool command entry만 남겨 90줄 이하로 유지합니다." \
  90 \
  "$PACKAGE_SOURCES/MainWindowFeature/MainWindow/Features/MainWindowFeature+DeveloperTools.swift"
